...
main.py
-----------
Skinalyze AI - Core API Application

This is the primary entry point for the Skinalyze backend. It replaces legacy OpenCV
heuristics with a state-of-the-art, 4-stage AI pipeline:
1. Universal Gatekeeper (CLIP): Validates the image is actually human skin.
2. Cosmetic Profiler (CLIP): Extracts skin tone, age, and minor cosmetic conditions.
3. Medical Specialist (PyTorch ResNet50): Scans for Melenoma, BCC, and Nevi.
4. LLM Synthesizer (DeepSeek): Generates personalized, empathetic medical responses.
"""
...
# Tell Hugging Face to NEVER look for TensorFlow
...
async def check_skin(
    ...
    try:
        ...
        # Specifically check for minor cosmetic conditions
        minor_labels = [
        ...
        profile_data = {
            ...
            "minor_condition": minor_res[0]["label"],
        ...
        if disease_model is not None:
            ...
            # 4. Set the final Health Status based strictly on PyTorch model
            if top_class == "normal":
